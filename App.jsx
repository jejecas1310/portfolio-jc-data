...
// --- 1. DONNÉES STATIQUES (DÉCLARÉES EN PREMIER POUR ÉVITER TOUTE ERREUR) ---
...
const expertiseCases = [
  { title: "Automatisation de Reporting", impact: "Gain de temps : 90%", description: "Transformation d'une saisie manuelle quotidienne en un import automatisé via script VBA." },
...
// --- 2. CONFIGURATION FIREBASE (REMPLACEZ PAR VOS CLÉS SUR GITHUB) ---
          ...
          {examples.map((ex, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-8 items-start text-left">
              ...
              <div className="flex-1 text-left text-white">
                ...
                <p className="text-slate-400 leading-relaxed mb-4 italic text-left">"{String(ex.context)}"</p>
...
const App = () => {
  ...
  // Authentification forcée (Rule 3)
  useEffect(() => {
  ...
  // Synchronisation Firestore (Rule 1 & 2) - Bloquée tant que user n'est pas OK
  useEffect(() => {
    if (!user || !db || !firebaseConfig.apiKey) return;

    // 1. Compteur Offre
    const giftRef = doc(db, 'artifacts', appId, 'public', 'data', 'stats', 'gift_counter');
    ...
    }, (err) => console.log("Firebase 9 Guard - Waiting..."));

    // 2. IA Usage
    const usageRef = doc(db, 'artifacts', appId, 'users', user.uid, 'ai_usage', 'stats');
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 text-left scroll-smooth">
        ...
        currentPage === 'home' ? (
          <>
            ...
            <section id="express" className="py-24 bg-slate-900 overflow-hidden relative scroll-mt-20 text-white text-left text-white">
              <div className="max-w-7xl mx-auto px-6 relative z-10 text-left text-white text-left text-white">
                <div className="mb-16 text-white text-left">
                  ...
                  <p className="text-slate-400 max-w-2xl border-l-4 border-amber-400 pl-6 text-left italic leading-relaxed text-white text-left font-bold">Pour des besoins ciblés et urgents, j'ai standardisé ces prestations sur la plateforme de confiance ComeUp.</p>
                </div>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 text-slate-900 text-left">
                  {standardServices.map((s) => (
                    <div key={s.id} className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 flex flex-col hover:bg-white/10 transition-all group text-left text-white text-left text-white">
                      ...
                      <p className="text-slate-400 text-xs mb-6 leading-relaxed italic text-left">{String(s.description)}</p>
                      <div className="pt-6 border-t border-white/10 flex items-center justify-between text-left">
                        <div className="text-xs text-white/50 italic text-left text-white">Forfait dès <span className="block text-lg font-black text-amber-400 uppercase tracking-tighter text-left">{String(s.price)}</span></div>
                        <a href={s.url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-amber-500 rounded-full flex items-center justify-center text-slate-900 shadow-lg hover:scale-110 transition-all text-slate-900"><ShoppingBag size={20} /></a>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section id="expertise" className="py-24 bg-white scroll-mt-20 text-left text-slate-900 text-left">
              <div className="max-w-7xl mx-auto px-6 text-left text-slate-900 text-left text-slate-900">
                <div className="mb-16 border-l-4 border-emerald-600 pl-8 text-left text-slate-900 text-left text-slate-900">
                  <h2 className="text-base font-bold text-emerald-600 uppercase tracking-widest mb-3 italic text-left text-emerald-600 text-left text-left text-left text-emerald-600">Cas d'Usage</h2>
                  <p className="text-3xl lg:text-4xl font-bold italic text-slate-900 text-left text-left text-left text-slate-900 text-left">Démonstration d'Expertise</p>
                  <p className="text-slate-500 mt-4 italic max-w-2xl text-left text-slate-600 leading-relaxed italic text-left text-left text-left text-left font-black">Scénarios types de transformation de problématiques métier en solutions techniques performantes.</p>
                </div>
                <div className="grid md:grid-cols-3 gap-8 text-left text-white text-left">
                  {expertiseCases.map((p, i) => (
                    <div key={i} className="bg-slate-50 rounded-[2.5rem] shadow-sm border border-slate-100 p-10 hover:shadow-xl transition-all group text-left text-slate-900 text-left">
                      <div className="flex items-center gap-2 text-emerald-600 font-black text-xs uppercase mb-4 tracking-tighter italic text-left text-emerald-600 text-left"><CheckCircle2 size={16} /> {String(p.impact)}</div>
                      <h3 className="text-2xl font-bold mb-4 group-hover:text-emerald-600 transition-colors uppercase italic tracking-tighter text-left text-slate-900 text-left">{String(p.title)}</h3>
                      <p className="text-slate-600 leading-relaxed italic text-sm text-left text-slate-600">"{String(p.description)}"</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <section id="methodology" className="py-24 bg-emerald-600 text-white relative italic text-center scroll-mt-20 text-white">
               <div className="absolute top-0 left-0 p-12 opacity-10 text-white text-left text-white"><Quote size={120} /></div>
               <div className="max-w-7xl mx-auto px-6 relative z-10 text-center text-white text-center text-center text-white">
                 <div className="mb-16 text-center text-white text-center text-center text-white">
                    <h2 className="text-base font-bold text-emerald-100 uppercase tracking-widest mb-3 italic text-center text-emerald-100 text-center font-black">Valeurs</h2>
                    <p className="text-3xl lg:text-5xl font-black italic tracking-tight uppercase text-center text-white text-center text-white">Engagement & Qualité</p>
                 </div>
                 <div className="grid md:grid-cols-3 gap-8 text-white text-left text-white text-left">
                    {methodology.map((m, i) => (
                      <div key={i} className="bg-white rounded-[2.5rem] p-10 shadow-2xl transform hover:-translate-y-2 transition-all text-slate-900 text-center flex flex-col items-center justify-center text-left text-slate-900 text-left">
                        ...
                        <h3 className="font-black text-slate-900 text-xl uppercase tracking-tighter mb-4 italic text-center text-slate-900">{String(m.title)}</h3>
                        <p className="text-slate-600 italic text-sm leading-relaxed text-center text-slate-600">"{String(m.content)}"</p>
                      </div>
                    ))}
                 </div>
               </div>
      ...
      {/* FOOTER */}
      <footer id="contact" className="py-24 bg-slate-900 text-white text-left italic text-white text-left">
        <div className="max-w-7xl mx-auto px-6 text-left text-white">
          <div className="grid md:grid-cols-2 gap-16 mb-20 text-left text-white text-left text-white">
            <div className="text-left text-white text-left text-left text-white">
              <h2 className="text-4xl font-bold mb-6 tracking-tight font-black uppercase leading-tight italic text-left text-white text-left text-white font-black">Optimisez votre gestion. <br/><span className="text-emerald-500 underline decoration-emerald-500/20 underline-offset-8 italic text-left text-emerald-500 text-left text-emerald-500">Commencez dès aujourd'hui.</span></h2>
              <div className="grid sm:grid-cols-2 gap-6 mt-12 text-white italic text-left text-left text-left text-white text-left">
                <a href="mailto:jc.data.solutions@outlook.fr" className="bg-white/5 p-6 rounded-2xl hover:bg-emerald-600 transition-all border border-white/5 group shadow-xl text-left text-white text-left text-white text-left text-white">
                    <Mail className="mb-4 text-emerald-400 group-hover:text-white text-emerald-400" />
                    <div className="truncate text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white font-black text-left text-white text-left text-white font-black">Email</div>
                    <div className="truncate text-sm font-black text-white font-black text-left text-white font-black text-white">jc.data.solutions@outlook.fr</div>
                </a>
                <a href="https://www.linkedin.com/in/j%C3%A9r%C3%B4me-cassier-511601324/" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-blue-700 transition-all border border-white/5 group shadow-xl text-left text-white text-left text-white text-left text-white font-black">
                    <Linkedin className="mb-4 text-blue-400 group-hover:text-white text-blue-400" />
                    <div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white font-black text-left text-white text-left text-white font-black text-left text-white">LinkedIn</div>
                    <div className="text-sm font-black uppercase tracking-tighter italic text-white font-black text-left text-white font-black text-left text-white">Profil Pro</div>
                </a>
                <a href="https://comeup.com/fr/@jerome-cassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-amber-500 transition-all border border-white/5 group shadow-lg text-left text-white text-left text-white text-left text-white">
                    <ShoppingBag className="mb-4 text-amber-500 group-hover:text-white text-amber-500" />
                    <div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white uppercase font-black text-left text-white font-black text-left text-white text-left text-white">Prestations</div>
                    <div className="text-sm font-black italic uppercase text-white font-black text-white text-left font-black text-left text-white text-white">Profil ComeUp</div>
                </a>
                <a href="https://www.malt.fr/profile/jeromecassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-red-600 transition-all border border-white/5 group shadow-lg text-left text-white text-left text-white text-left text-white text-left text-white">
                    <UserCheck className="mb-4 text-red-500 group-hover:text-white text-red-500" />
                    <div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white uppercase font-black text-left text-white font-black text-left text-left text-white text-left text-white">Indépendant</div>
                    <div className="text-sm font-black uppercase tracking-tighter text-white font-black text-white text-left font-black text-left text-white text-white">Profil Malt</div>
                </a>
              </div>
            </div>
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden text-slate-900 text-left italic text-slate-900 text-left text-left text-slate-900 text-left text-left font-black">
              {isSubmitted ? (
                <div className="text-center py-8 text-slate-900 text-center text-left text-center text-center text-slate-900">
                  <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6 text-center text-emerald-600 text-left text-left text-center text-emerald-600"><CheckCircle2 size={40} /></div>
                  <h3 className="text-2xl font-bold mb-4 font-black uppercase tracking-tight italic text-slate-900 text-center text-slate-900 text-center text-left text-slate-900 text-center text-left font-black text-slate-900">Demande reçue !</h3>
                  <p className="text-slate-500 text-sm text-center italic text-center text-slate-500 text-center text-center text-center font-black text-slate-500">Réponse sous 24h maximum.</p>
                ...
                <form className="space-y-4 text-left text-slate-900 text-left text-left text-left text-left text-left text-left text-left text-left text-slate-900 font-black text-slate-900" onSubmit={handleContactSubmit}>
                  <h3 className="text-2xl font-bold mb-8 font-black tracking-tight text-left uppercase italic text-slate-900 italic underline decoration-emerald-500/20 text-left text-left text-left text-left text-left text-left font-black text-slate-900">Demander un audit gratuit</h3>
                  ...
                  <button type="submit" disabled={isSending} className="w-full bg-emerald-600 text-white font-black py-5 rounded-xl hover:bg-emerald-700 shadow-xl transition-all flex items-center justify-center gap-3 italic uppercase tracking-tighter text-white text-left text-white font-black text-white">
          ...
          <div className="pt-12 border-t border-white/10 flex items-center justify-between text-white text-xs text-left text-white text-left text-left text-left text-left text-white font-black text-white">
            <div className="flex items-center gap-2 text-left text-white text-white">
              <Database className="text-emerald-500 w-5 h-5 text-left text-emerald-500 text-left text-left text-emerald-500 text-left text-emerald-500" />
              <span className="font-black text-white uppercase italic tracking-tighter text-left text-white text-left text-white text-left text-white text-left text-white">JC.DATA<span className="text-emerald-600">.SOLUTIONS</span></span>
              
              {/* BOUTON ADMIN JAUNE AMBRE (VIF) */}
              <button onClick={() => setCurrentPage('admin')} 
                className="ml-4 p-2 bg-white/5 border border-amber-400/30 rounded-lg text-amber-400 hover:bg-amber-400 hover:text-slate-900 transition-all shadow-lg text-amber-400">
                <Lock size={16} />
              </button>
            </div>
            <p className="italic uppercase tracking-widest text-[9px] italic font-bold text-left text-white font-black text-left text-white text-white text-left text-white text-left text-white">© {new Date().getFullYear()} - Expert Automation & Intelligence de Données.</p>
          </div>
        </div>
      </footer>
      ...
      {/* IA MODAL */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm text-slate-900 italic text-left text-left text-slate-900 text-slate-900">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 text-slate-900 text-slate-900">
              <div className="text-left text-slate-900 text-left text-left text-left font-black text-slate-900">
                <h3 className="text-2xl font-bold flex items-center gap-2 font-black tracking-tight uppercase italic text-left text-slate-900 text-left text-left text-left text-slate-900 text-left text-left text-left text-slate-900"><Sparkles className="text-emerald-600 text-left text-emerald-600 text-left text-left font-black text-emerald-600" /> Pré-Diagnostic IA ✨</h3>
                <div className="flex items-center gap-2 mt-1 text-left text-left text-slate-200 text-left text-left text-slate-200">
                  <div className="h-1.5 w-32 bg-slate-200 rounded-full overflow-hidden text-left text-left text-slate-200">
                    <div className="h-full bg-emerald-500 transition-all duration-700 text-left text-left text-emerald-500" style={{ width: `${(Number(aiUsageCount)/3)*100}%` }}></div>
                  </div>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left italic text-left text-left text-left text-slate-400 text-slate-400">{aiUsageCount}/3 diagnostics gratuits</span>
                </div>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors text-left text-left text-left text-left font-black text-slate-400 text-slate-400"><X size={24} /></button>
            </div>
            <div className="p-8 overflow-y-auto text-left italic text-slate-900 text-left text-left text-left text-left text-left text-left text-slate-900 font-black text-slate-900">
              {!aiResult ? (
                aiUsageCount >= 3 ? (
                  <div className="text-center py-6 animate-in zoom-in duration-500 text-slate-900 text-center text-left text-left text-slate-900 text-left text-left font-black text-center flex flex-col items-center text-slate-900">
                    <div className="w-20 h-20 bg-amber-100 text-amber-600 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-inner text-amber-600 text-center flex items-center justify-center text-center text-left text-left font-black text-amber-600"><Gift size={40} /></div>
                    <h3 className="text-3xl font-black text-slate-900 mb-4 tracking-tight text-center italic uppercase text-center text-slate-900 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-slate-900">L'IA a ses limites 🚀</h3>
                    <p className="text-slate-600 mb-10 max-w-md mx-auto text-lg leading-relaxed text-center italic font-bold text-center text-slate-600 text-left text-left text-left text-left text-left font-black text-slate-600">Votre projet mérite une expertise humaine directe.</p>
                    <button onClick={() => { setIsModalOpen(false); navigateToSection('contact'); }} className="bg-emerald-600 text-white px-8 py-5 rounded-2xl font-black text-xl hover:scale-105 transition-all shadow-xl shadow-emerald-100 flex items-center justify-center gap-3 w-full uppercase tracking-tighter text-white text-left text-left font-black text-center text-white">Réserver mon audit gratuit <ArrowRight /></button>
                  </div>
                ) : (
                  <div className="space-y-4 text-left text-slate-900 text-left text-left text-slate-900 text-left text-left text-left text-left text-left text-slate-900 text-left font-black text-slate-900 text-slate-900">
                    <div className="flex items-center gap-2 text-emerald-600 font-bold text-xs uppercase tracking-widest mb-2 italic text-emerald-600 text-left text-left text-left text-left text-left font-black text-emerald-600 text-emerald-600"><Info size={14}/> Décrivez votre flux de travail actuel</div>
                    ...
                    <button onClick={handleAiAction} disabled={isLoading || !userInput.trim()} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg hover:bg-emerald-600 transition-all flex items-center justify-center gap-3 shadow-lg uppercase tracking-tighter italic text-white text-white text-left font-black text-center text-white">
                      {isLoading ? <Loader2 className="animate-spin text-white" /> : <><Sparkles size={20} /> Analyser mon besoin</>}
                    </button>
                  </div>
                )
              ) : (
                <div className="space-y-6 text-left text-slate-900 text-left text-left text-slate-900 text-left text-left text-left text-left text-left text-slate-900 text-left font-black text-slate-900 text-slate-900">
                  <div className="bg-emerald-50 p-8 rounded-[2rem] border border-emerald-100 whitespace-pre-wrap leading-relaxed text-left shadow-sm text-sm italic text-slate-800 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left text-left text-left font-black text-emerald-700 text-emerald-700">
                    <div className="text-emerald-700 font-bold mb-4 flex items-center gap-2 border-b border-emerald-100 pb-2 uppercase text-xs tracking-widest not-italic italic underline decoration-emerald-200 italic font-black text-left text-emerald-700 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left text-left text-left text-left text-left font-black text-emerald-700"><Sparkles size={14}/> Analyse Préliminaire</div>
                    {String(aiResult)}
                  </div>
                  <div className="p-6 bg-slate-900 rounded-[2rem] text-white flex flex-col sm:flex-row items-center gap-6 shadow-xl text-left text-white text-left text-left text-left text-left text-left text-left text-left text-left text-white text-left text-left text-left text-left text-left text-left text-left font-black text-white text-white">
                    <div className="flex-1 text-left text-white text-left text-left text-left text-left text-white text-left text-left text-left text-left text-white text-left text-left text-left text-left text-white text-left text-white text-left text-left font-black text-white">
                        <div className="text-emerald-400 font-black uppercase text-xs tracking-widest mb-1 italic text-left text-emerald-400 font-black text-left text-left text-left text-left text-emerald-400 text-left text-left text-left text-left font-black text-emerald-400">Valider cette analyse ?</div>
                        <p className="text-xs text-slate-400 italic leading-relaxed text-left text-slate-400 text-left text-left text-left text-left text-left text-left text-left text-slate-400 text-left text-left text-left text-left text-left text-left text-left text-slate-400 text-left text-left text-left text-left text-left text-left text-left text-slate-400 font-black text-slate-400">Échangeons 15 minutes pour confirmer le budget et la faisabilité.</p>
                    </div>
                    <button onClick={() => { setIsModalOpen(false); navigateToSection('contact'); }} className="bg-emerald-600 text-white px-6 py-4 rounded-xl font-black text-sm uppercase flex items-center gap-2 hover:bg-emerald-500 transition-all whitespace-nowrap italic text-white text-left text-left text-left text-left text-left text-left text-left text-left font-black text-center text-white"><MessageSquare size={16}/> Demander mon audit <ArrowRight size={16}/></button>
                  </div>
                  <button onClick={() => setAiResult("")} className="w-full py-3 text-slate-400 text-[10px] font-bold uppercase tracking-widest hover:text-slate-600 transition-colors italic underline italic text-center block w-full text-slate-400 text-left text-left text-left text-left text-left text-slate-400 text-left text-left text-left text-left text-left font-black text-center text-slate-400">Autre analyse ({3 - aiUsageCount} restantes)</button>
                </div>
              )}
            </div>
          </div>

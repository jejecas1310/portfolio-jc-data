...
import { 
  ...
  Layers, Layout, AlertCircle, Timer, Send, Lock, Trash2, Calendar, Info, Gift, Star, Quote, ChevronDown, ChevronUp, MessageSquare, Target, Shield, Clock
} from 'lucide-react';
...
const App = () => {
  ...
  // --- GESTION DU SCROLL AU CHANGEMENT DE VUE ---
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [currentPage]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 scroll-smooth selection:bg-emerald-100 text-left">
      {/* NAVIGATION */}
      ...
      {currentPage === 'admin' ? (
        <div className="pt-32 pb-24 min-h-screen max-w-6xl mx-auto px-6 text-left">
          ...
          <h1 className="text-4xl font-black mb-8 italic uppercase tracking-tight text-left text-slate-900">Espace Admin</h1>
          {!isAdminAuthenticated ? (
            ...
            <div className="space-y-8 text-slate-900">
              <h2 className="text-2xl font-bold uppercase italic text-left">Demandes Prospects ({leads.length})</h2>
        ...
        currentPage === 'home' ? (
          <>
              ...
              <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
                <div className="text-left text-slate-900">
                  ...
                  <div className="flex flex-col gap-4 items-start text-left">
                    ...
                    <p className="text-slate-400 text-xs italic ml-2 max-w-md text-left">Obtenez instantanément une stratégie technique et une estimation de faisabilité pour vos besoins d'automatisation.</p>
                  </div>
                </div>
                   ...
                   <div className="relative bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-8 transform rotate-2 inline-block">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-emerald-50 rounded-2xl p-6 text-center text-slate-900">
                        ...
                        <div className="text-[10px] text-emerald-800 font-bold uppercase tracking-tighter italic text-center">Temps manuel</div>
                      </div>
                      <div className="bg-slate-900 rounded-2xl p-6 text-white text-center">
                        ...
                        <div className="text-2xl font-black italic text-center">0</div>
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter italic text-center text-white">Erreur Saisie</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <section id="services" className="py-24 bg-slate-50 scroll-mt-20">
              <div className="max-w-7xl mx-auto px-6 text-left text-slate-900">
                <div className="max-w-2xl mb-16 border-l-4 border-emerald-600 pl-8 text-left">
                  ...
                  <p className="text-3xl lg:text-4xl font-bold italic text-left text-slate-900">
                    Solutions d'ingénierie Excel <span className="whitespace-nowrap italic">sur-mesure.</span>
                  </p>
                </div>
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 text-left text-slate-900">
                  ...
                  <ServiceCard icon={BarChart3} title="Data Visualisation" desc="Conception de tableaux de bord dynamiques et visuels pour vos KPI." onClick={() => setCurrentPage('details-dashboards')} color="blue" />
            ...
            <section id="express" className="py-24 bg-slate-900 overflow-hidden relative scroll-mt-20 text-white text-left">
              <div className="max-w-7xl mx-auto px-6 relative z-10 text-left">
                <div className="mb-16 text-left">
                  ...
                  <p className="text-slate-400 max-w-2xl border-l-4 border-amber-400 pl-6 text-left italic leading-relaxed">Pour des besoins ciblés et urgents, j'ai standardisé ces prestations sur la plateforme de confiance ComeUp.</p>
                </div>
                  ...
                  {standardServices.map((s) => (
                    <div key={s.id} className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 flex flex-col hover:bg-white/10 transition-all relative overflow-hidden group">
                      ...
                      <div className="pt-6 border-t border-white/10 flex items-center justify-between">
                        <div className="text-xs text-white/50 italic text-left">Forfait dès <span className="block text-lg font-black text-amber-400 uppercase tracking-tighter">{s.price}</span></div>
            ...
            <section id="expertise" className="py-24 bg-white scroll-mt-20">
              <div className="max-w-7xl mx-auto px-6 text-slate-900 text-left">
                <div className="mb-16 text-left border-l-4 border-emerald-600 pl-8">
                  ...
                  <p className="text-3xl lg:text-4xl font-bold italic text-left text-slate-900">Démonstration d'Expertise</p>
                  <p className="text-slate-500 mt-4 italic max-w-2xl italic leading-relaxed text-left text-slate-600">Scénarios types de transformation de problématiques métier en solutions techniques performantes.</p>
                </div>
                <div className="grid md:grid-cols-3 gap-8 text-left">
                  {expertiseCases.map((p, i) => (
                    <div key={i} className="bg-slate-50 rounded-[2.5rem] shadow-sm border border-slate-100 p-10 hover:shadow-xl transition-all group text-slate-900">
            ...
            {/* ENGAGEMENT */}
            <section id="methodology" className="py-24 bg-emerald-600 scroll-mt-20 text-white relative italic text-center">
               ...
               <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
                 ...
                 <div className="grid md:grid-cols-3 gap-8 text-left">
                    {methodology.map((m, i) => (
                      <div key={i} className="bg-white rounded-[2.5rem] p-10 shadow-2xl transform hover:-translate-y-2 transition-all text-slate-900 text-center flex flex-col items-center justify-center">
                        <div className="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-3xl flex items-center justify-center mb-6 shadow-sm"><m.icon size={32} /></div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm text-slate-900 italic">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="text-left text-slate-900">
                <h3 className="text-2xl font-bold flex items-center gap-2 font-black tracking-tight uppercase italic text-left"><Sparkles className="text-emerald-600" /> Pré-Diagnostic IA ✨</h3>
              ...
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors text-left"><X size={24} /></button>
            </div>
            <div className="p-8 overflow-y-auto text-left italic">
              {!aiResult ? (
                aiUsageCount >= 3 ? (
                  <div className="text-center py-6 animate-in zoom-in duration-500 text-left">
                    ...
                    <h3 className="text-3xl font-black text-slate-900 mb-4 tracking-tight text-center italic uppercase italic">L'IA a ses limites 🚀</h3>
                    <p className="text-slate-600 mb-10 max-w-md mx-auto text-lg leading-relaxed text-center italic font-bold">Votre projet mérite une expertise humaine directe.</p>
                  ...
                  <div className="space-y-4 text-left">
                    <div className="flex items-center gap-2 text-emerald-600 font-bold text-xs uppercase tracking-widest mb-2 italic text-left text-slate-900"><Info size={14}/> Décrivez votre flux de travail actuel</div>
                    <textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-emerald-500 outline-none text-lg min-h-[160px] text-left italic text-slate-900" placeholder="Ex: Chaque lundi, je dois consolider 10 fichiers..."></textarea>
                ...
                <div className="space-y-6 text-left text-slate-900">
                  <div className="bg-emerald-50 p-8 rounded-[2rem] border border-emerald-100 whitespace-pre-wrap leading-relaxed text-left text-slate-800 shadow-sm text-sm italic text-left text-slate-900">
                    <div className="text-emerald-700 font-bold mb-4 flex items-center gap-2 border-b border-emerald-100 pb-2 uppercase text-xs tracking-widest not-italic italic underline decoration-emerald-200 italic font-black text-left"><Sparkles size={14}/> Analyse Préliminaire</div>
                    {aiResult}
                  </div>
                  ...
                  <button onClick={() => setAiResult("")} className="w-full py-3 text-slate-400 text-[10px] font-bold uppercase tracking-widest hover:text-slate-600 transition-colors italic underline italic text-center block w-full">Autre analyse ({3 - aiUsageCount} restantes)</button>
                </div>
              )}
            </div>
          </div>
      ...
      <footer id="contact" className="py-24 bg-slate-900 text-white scroll-mt-20 text-left italic">
        <div className="max-w-7xl mx-auto px-6 text-left">
          <div className="grid md:grid-cols-2 gap-16 mb-20 text-left italic text-white text-left">
            <div className="text-left text-left">
              <h2 className="text-4xl font-bold mb-6 tracking-tight font-black uppercase leading-tight italic text-left text-white text-left">Optimisez votre gestion. <br/><span className="text-emerald-500 underline decoration-emerald-500/20 underline-offset-8 italic text-left">Commencez dès aujourd'hui.</span></h2>
              <div className="grid sm:grid-cols-2 gap-6 mt-12 text-white italic text-left text-left">
                <a href="mailto:jc.data.solutions@outlook.fr" className="bg-white/5 p-6 rounded-2xl hover:bg-emerald-600 transition-all border border-white/5 group shadow-xl text-left text-left"><Mail className="mb-4 text-emerald-400 group-hover:text-white text-left" /><div className="truncate text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white text-left text-left font-black">Email</div><div className="truncate text-sm font-black text-left text-left text-white font-black">jc.data.solutions@outlook.fr</div></a>
                <a href="https://www.linkedin.com/in/j%C3%A9r%C3%B4me-cassier-511601324/" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-blue-700 transition-all border border-white/5 group shadow-xl text-left text-left"><Linkedin className="mb-4 text-blue-400 group-hover:text-white text-left" /><div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white text-left text-left font-black">LinkedIn</div><div className="text-sm font-black uppercase tracking-tighter italic text-white italic text-left text-left text-white font-black">Profil Pro</div></a>
                <a href="https://comeup.com/fr/@jerome-cassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-amber-500 transition-all border border-white/5 group shadow-lg text-left text-left text-white"><ShoppingBag className="mb-4 text-amber-500 group-hover:text-white text-left" /><div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white uppercase italic text-left text-left font-black">Prestations</div><div className="text-sm font-black italic uppercase text-white italic text-left text-left text-white font-black">Profil ComeUp</div></a>
                <a href="https://www.malt.fr/profile/jeromecassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-red-600 transition-all border border-white/5 group shadow-lg text-left text-left text-white"><UserCheck className="mb-4 text-red-500 group-hover:text-white text-left" /><div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white uppercase italic text-left text-left font-black">Indépendant</div><div className="text-sm font-black uppercase tracking-tighter text-white italic text-left text-left text-white font-black">Profil Malt</div></a>
              </div>
            </div>
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden text-slate-900 text-left italic">
              {isSubmitted ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-2xl font-bold mb-4 font-black uppercase tracking-tight italic text-slate-900 text-center text-center">Demande reçue !</h3>
                  <p className="text-slate-500 text-sm text-center italic text-center text-center">Réponse sous 24h maximum.</p>
                  <button onClick={() => setIsSubmitted(false)} className="mt-8 text-emerald-600 font-bold hover:underline italic uppercase tracking-widest text-xs italic text-center block w-full">Envoyer un autre message</button>
                </div>
              ) : (
                <form className="space-y-4 text-left text-slate-900" onSubmit={handleContactSubmit}>
                  <h3 className="text-2xl font-bold mb-8 font-black tracking-tight text-left uppercase italic text-slate-900 italic underline decoration-emerald-500/20 text-left text-left">Demander un audit gratuit</h3>
                  <input required value={contactForm.name} onChange={(e) => setContactForm({...contactForm, name: e.target.value})} type="text" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-left text-slate-900" placeholder="Nom complet" />
                  <input required value={contactForm.email} onChange={(e) => setContactForm({...contactForm, email: e.target.value})} type="email" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-left text-slate-900" placeholder="Email professionnel" />
                  <textarea required value={contactForm.message} onChange={(e) => setContactForm({...contactForm, message: e.target.value})} rows="4" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-left text-slate-900" placeholder="Décrivez votre besoin métier..."></textarea>
          ...
          <div className="pt-12 border-t border-white/10 flex items-center justify-between text-slate-500 text-xs text-left text-white">
            <div className="flex items-center gap-2 text-left">
              <Database className="text-emerald-500 w-5 h-5 text-left text-emerald-500" />
              <span className="font-black text-white uppercase italic tracking-tighter italic text-left text-left text-white">JC.DATA<span className="text-emerald-600">.SOLUTIONS</span></span>
              <button onClick={() => setCurrentPage('admin')} className="ml-4 opacity-10 hover:opacity-100 text-white transition-opacity italic text-left text-left text-white"><Lock size={12} /></button>
            </div>
            <p className="italic uppercase tracking-widest text-[9px] italic font-bold text-left text-left text-white">© {new Date().getFullYear()} - Expert Automation & Intelligence de Données.</p>
          </div>
        </div>
      </footer>
    </div>
...
const DetailsTemplate = ({ title, subtitle, description, icon: Icon, examples, colorClass, goBack }) => (
  <div className="pt-32 pb-24 animate-in fade-in duration-700 bg-white min-h-screen text-slate-900 text-left italic text-left text-left text-left">
    <div className="max-w-4xl mx-auto px-6 text-left">
      <button onClick={goBack} className="flex items-center gap-2 text-slate-500 hover:text-emerald-600 font-bold text-sm mb-12 group transition-all italic text-left text-left">
        <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Retour à l'accueil
      </button>
      <div className="mb-16 text-left text-left">
        <div className={`inline-flex items-center gap-2 px-3 py-3 rounded-2xl bg-${colorClass}-100 text-${colorClass}-700 mb-6 text-left text-left`}><Icon size={40} /></div>
        <h1 className="text-4xl lg:text-6xl font-black text-slate-900 mb-6 leading-tight text-left uppercase italic text-left text-left text-left text-slate-900">{title} : <br/><span className={`text-${colorClass}-600`}>{subtitle}</span></h1>
        <p className="text-xl text-slate-600 leading-relaxed text-left italic leading-relaxed text-left text-left text-left text-slate-600">{description}</p>
      </div>
      <div className="bg-slate-900 rounded-[3rem] p-10 md:p-16 text-white mb-12 shadow-2xl border border-white/5 text-left italic text-left text-left text-white">
        ...
        <div className="space-y-16 text-left text-left text-left text-left">
          {examples.map((ex, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-8 items-start text-left text-left text-left">
              <div className={`w-12 h-12 bg-${colorClass}-500 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-${colorClass}-900/20 text-left text-left text-left`}><Layout size={24} className="text-white text-left text-left text-left" /></div>
              <div className="flex-1 text-left text-left text-left text-left">
                <h4 className={`text-xl font-bold mb-3 text-${colorClass}-400 uppercase tracking-tighter italic text-left text-left text-left text-white`}>{i + 1}. {ex.title}</h4>
                <p className="text-slate-400 leading-relaxed mb-4 text-left italic text-left text-left text-left text-slate-400">"{ex.context}"</p>
                <div className="bg-white/5 p-6 rounded-2xl border border-white/10 text-left text-left text-left">
                  <span className={`text-${colorClass}-500 font-black block mb-2 uppercase text-xs tracking-widest italic text-left text-left text-left`}>Approche Technique :</span>
                  <span className="text-slate-200 text-left block leading-relaxed italic text-left text-left text-left text-slate-200">{ex.solution}</span>
                </div>
              </div>
            </div>
          ))}

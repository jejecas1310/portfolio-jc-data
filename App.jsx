            ...
            <div key={i} className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1 text-left">
                ...
                <div className="bg-white/5 p-6 rounded-2xl border border-white/10 text-white">
                  ...
                  <span className="text-slate-200 block leading-relaxed italic">{ex.solution}</span>
                </div>
              </div>
            </div>
          ))}
...
const App = () => {
  ...
  // Firestore Sync Manager (Rule 1 & 2)
  useEffect(() => {
    if (!user || !db) return;

    // Compteur Cadeau
    const settingsRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'offer');
    ...
    // Usage IA
    const usageDocRef = doc(db, 'artifacts', appId, 'users', user.uid, 'stats', 'ai');
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          ...
          systemInstruction: { parts: [{ text: "Assistant expert VBA. Analyse pro et invitation audit 15min." }] }
        })
      });
      ...
      if (db && user) {
        await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'stats', 'ai'), { count: increment(1), lastUsed: serverTimestamp() }, { merge: true });
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 text-left scroll-smooth">
        ...
        currentPage === 'home' ? (
          <>
            ...
            <section className="relative pt-64 pb-20 lg:pt-80 lg:pb-32 overflow-hidden bg-white text-left">
              ...
              <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
                <div className="text-left text-slate-900">
                  <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-black uppercase tracking-widest mb-6 border border-amber-200 italic shadow-sm">
                    <Gift size={12} /> Pack Cadeau Dispo ({currentGiftCount}/10)
                  </div>
                  <h1 className="text-5xl lg:text-7xl font-extrabold leading-[1.1] mb-8 font-black uppercase tracking-tight italic">Libérez la puissance de vos données <span className="text-emerald-600">Excel</span></h1>
                  ...
                  <div className="flex flex-col gap-4 items-start text-left">
                    <button onClick={() => setIsModalOpen(true)} className="bg-emerald-600 text-white px-8 py-4 rounded-xl font-bold hover:bg-emerald-700 transition-all shadow-lg inline-flex items-center gap-2 uppercase tracking-tight italic text-lg tracking-widest">Analyser mon projet (IA) ✨</button>
            ...
            <section id="services" className="py-24 bg-slate-50 scroll-mt-20 text-left text-slate-900">
              <div className="max-w-7xl mx-auto px-6 text-left">
                <div className="max-w-2xl mb-16 border-l-4 border-emerald-600 pl-8 text-left">
                  ...
                  <p className="text-3xl lg:text-4xl font-bold italic text-slate-900 text-left">Solutions d'ingénierie Excel <span className="whitespace-nowrap italic text-slate-900">sur-mesure.</span></p>
                </div>
            ...
            <section id="express" className="py-24 bg-slate-900 overflow-hidden relative scroll-mt-20 text-white text-left">
              <div className="max-w-7xl mx-auto px-6 relative z-10 text-left text-white">
                <div className="mb-16">
                  ...
                  <h2 className="text-base font-bold text-amber-400 uppercase tracking-widest mb-3 italic text-left text-amber-400">Forfaits d'Intervention</h2>
                  ...
                  <p className="text-slate-400 max-w-2xl border-l-4 border-amber-400 pl-6 text-left italic leading-relaxed text-white">Pour des besoins ciblés et urgents, j'ai standardisé ces prestations sur la plateforme de confiance ComeUp.</p>
                </div>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 text-slate-900">
                  {standardServices.map((s) => (
                    <div key={s.id} className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 flex flex-col hover:bg-white/10 transition-all group text-left text-white">
                      ...
                      <div className="pt-6 border-t border-white/10 flex items-center justify-between text-left text-white">
                        <div className="text-xs text-white/50 italic text-left">Forfait dès <span className="block text-lg font-black text-amber-400 uppercase tracking-tighter text-left">{s.price}</span></div>
            ...
            <section id="expertise" className="py-24 bg-white scroll-mt-20 text-left text-slate-900">
              <div className="max-w-7xl mx-auto px-6 text-left">
                <div className="mb-16 border-l-4 border-emerald-600 pl-8 text-left">
                  <h2 className="text-base font-bold text-emerald-600 uppercase tracking-widest mb-3 italic text-left text-emerald-600">Cas d'Usage</h2>
                  <p className="text-3xl lg:text-4xl font-bold italic text-slate-900 text-left">Démonstration d'Expertise</p>
                  <p className="text-slate-500 mt-4 italic max-w-2xl text-left text-slate-600 leading-relaxed italic">Scénarios types de transformation de problématiques métier en solutions techniques performantes.</p>
                </div>
            ...
            <section id="methodology" className="py-24 bg-emerald-600 text-white relative italic text-center scroll-mt-20">
               ...
               <div className="max-w-7xl mx-auto px-6 relative z-10 text-center text-white">
                 <div className="mb-16 text-center text-white text-center">
                    <h2 className="text-base font-bold text-emerald-100 uppercase tracking-widest mb-3 italic text-center text-emerald-100">Valeurs</h2>
                    <p className="text-3xl lg:text-5xl font-black italic tracking-tight uppercase text-center text-white">Engagement & Qualité</p>
                 </div>
        ...
        <div className="max-w-7xl mx-auto px-6 text-left">
          <div className="grid md:grid-cols-2 gap-16 mb-20 text-left text-white text-left">
            <div className="text-left text-white text-left">
              <h2 className="text-4xl font-bold mb-6 tracking-tight font-black uppercase leading-tight italic text-left text-white">Optimisez votre gestion. <br/><span className="text-emerald-500 underline decoration-emerald-500/20 underline-offset-8 italic text-left text-emerald-500">Commencez dès aujourd'hui.</span></h2>
              <div className="grid sm:grid-cols-2 gap-6 mt-12 text-white italic text-left text-left">
                <a href="mailto:jc.data.solutions@outlook.fr" className="bg-white/5 p-6 rounded-2xl hover:bg-emerald-600 transition-all border border-white/5 group shadow-xl text-left text-white text-left text-white">
                ...
                <a href="https://www.linkedin.com/in/j%C3%A9r%C3%B4me-cassier-511601324/" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-blue-700 transition-all border border-white/5 group shadow-xl text-left text-white text-left text-white">
                    <Linkedin className="mb-4 text-blue-400 group-hover:text-white" />
                    <div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white font-black text-left text-white font-black">LinkedIn</div>
                    <div className="text-sm font-black uppercase tracking-tighter italic text-white font-black text-left text-white font-black">Profil Pro</div>
                </a>
                <a href="https://comeup.com/fr/@jerome-cassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-amber-500 transition-all border border-white/5 group shadow-lg text-left text-white text-left text-white text-left">
                    ...
                    <div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white font-black text-left text-white font-black">Prestations</div>
                    <div className="text-sm font-black italic uppercase text-white font-black text-white text-left font-black">Profil ComeUp</div>
                </a>
                <a href="https://www.malt.fr/profile/jeromecassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-red-600 transition-all border border-white/5 group shadow-lg text-left text-white text-left text-white text-left">
                    <UserCheck className="mb-4 text-red-500 group-hover:text-white" />
                    <div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white uppercase font-black text-left text-white font-black">Indépendant</div>
                    <div className="text-sm font-black uppercase tracking-tighter text-white font-black text-white text-left font-black">Profil Malt</div>
                </a>
              </div>
            </div>
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden text-slate-900 text-left italic">
              {isSubmitted ? (
                <div className="text-center py-8 text-slate-900 text-center text-left">
                  <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6 text-center text-emerald-600 text-left"><CheckCircle2 size={40} /></div>
                  <h3 className="text-2xl font-bold mb-4 font-black uppercase tracking-tight italic text-slate-900 text-center text-slate-900 text-center text-left text-slate-900">Demande reçue !</h3>
                  <p className="text-slate-500 text-sm text-center italic text-center text-slate-500 text-center text-center">Réponse sous 24h maximum.</p>
                ...
                <form className="space-y-4 text-left text-slate-900 text-left text-left text-left text-left text-left text-left text-left text-slate-900" onSubmit={handleContactSubmit}>
                  <h3 className="text-2xl font-bold mb-8 font-black tracking-tight text-left uppercase italic text-slate-900 italic underline decoration-emerald-500/20 text-left text-left text-left text-left text-left">Demander un audit gratuit</h3>
          ...
          <div className="pt-12 border-t border-white/10 flex items-center justify-between text-white text-xs text-left text-white text-left text-left text-left text-white">
            <div className="flex items-center gap-2 text-left text-white">
              ...
              <span className="font-black text-white uppercase italic tracking-tighter text-left text-white text-left text-white">JC.DATA<span className="text-emerald-600">.SOLUTIONS</span></span>
      ...
      {/* IA MODAL */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm text-slate-900 italic text-left">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 text-left text-slate-900 text-left text-left">
              <div className="text-left text-slate-900">
                <h3 className="text-2xl font-bold flex items-center gap-2 font-black tracking-tight uppercase italic text-left text-slate-900 text-left text-left"><Sparkles className="text-emerald-600 text-left text-emerald-600" /> Pré-Diagnostic IA ✨</h3>
                <div className="flex items-center gap-2 mt-1 text-left text-left text-slate-200">
                  <div className="h-1.5 w-32 bg-slate-200 rounded-full overflow-hidden text-left text-left text-slate-200">
                    <div className="h-full bg-emerald-500 transition-all duration-700 text-left" style={{ width: `${(Number(aiUsageCount)/3)*100}%` }}></div>
                  </div>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left italic text-left">{aiUsageCount}/3 diagnostics gratuits</span>
                </div>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors text-left text-left"><X size={24} /></button>
            </div>
            <div className="p-8 overflow-y-auto text-left italic text-slate-900 text-left text-left text-left text-slate-900">
              {!aiResult ? (
                aiUsageCount >= 3 ? (
                  <div className="text-center py-6 animate-in zoom-in duration-500 text-slate-900 text-center text-left text-left text-slate-900">
                    <div className="w-20 h-20 bg-amber-100 text-amber-600 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-inner text-amber-600 text-center flex items-center justify-center text-center"><Gift size={40} /></div>
                    <h3 className="text-3xl font-black text-slate-900 mb-4 tracking-tight text-center italic uppercase text-center text-slate-900 text-left">L'IA a ses limites 🚀</h3>
                    <p className="text-slate-600 mb-10 max-w-md mx-auto text-lg leading-relaxed text-center italic font-bold text-center text-slate-600 text-left">Votre projet mérite une expertise humaine directe.</p>
                    <button onClick={() => { setIsModalOpen(false); navigateToSection('contact'); }} className="bg-emerald-600 text-white px-8 py-5 rounded-2xl font-black text-xl hover:scale-105 transition-all shadow-xl shadow-emerald-100 flex items-center justify-center gap-3 w-full uppercase tracking-tighter text-white">Réserver mon audit gratuit <ArrowRight /></button>
                  </div>
                ) : (
                  <div className="space-y-4 text-left text-slate-900 text-left text-left text-slate-900 text-left text-left text-left text-slate-900">
                    <div className="flex items-center gap-2 text-emerald-600 font-bold text-xs uppercase tracking-widest mb-2 italic text-emerald-600 text-left"><Info size={14}/> Décrivez votre flux de travail actuel</div>
                    <textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-emerald-500 outline-none text-lg min-h-[160px] text-left italic text-slate-900 text-left text-left text-left" placeholder="Ex: Chaque lundi, je fusionne 10 fichiers..."></textarea>
                    <button onClick={handleAiAction} disabled={isLoading || !userInput.trim()} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg hover:bg-emerald-600 transition-all flex items-center justify-center gap-3 shadow-lg uppercase tracking-tighter italic text-white text-white">
                      {isLoading ? <Loader2 className="animate-spin text-white" /> : <><Sparkles size={20} /> Analyser mon besoin</>}
                    </button>
                  </div>
                )
              ) : (
                <div className="space-y-6 text-left text-slate-900 text-left text-left text-slate-900 text-left text-left text-left text-slate-900">
                  <div className="bg-emerald-50 p-8 rounded-[2rem] border border-emerald-100 whitespace-pre-wrap leading-relaxed text-left shadow-sm text-sm italic text-slate-800 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left">
                    <div className="text-emerald-700 font-bold mb-4 flex items-center gap-2 border-b border-emerald-100 pb-2 uppercase text-xs tracking-widest not-italic italic underline decoration-emerald-200 italic font-black text-left text-emerald-700 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left text-left text-emerald-700"><Sparkles size={14}/> Analyse Préliminaire</div>
                    {String(aiResult)}
                  </div>
                  ...
                  <button onClick={() => setAiResult("")} className="w-full py-3 text-slate-400 text-[10px] font-bold uppercase tracking-widest hover:text-slate-600 transition-colors italic underline italic text-center block w-full text-slate-400 text-left text-left text-left text-left text-left text-slate-400 text-left text-left text-left text-left">Autre analyse ({3 - aiUsageCount} restantes)</button>
                </div>
              )}
            </div>
          </div>

    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 scroll-smooth selection:bg-emerald-100 text-left">
                   ...
                   <div className="relative bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-8 transform rotate-2 inline-block">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="bg-slate-900 rounded-2xl p-6 text-white text-center">
                        ...
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter italic text-center">Erreur Saisie</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <section id="express" className="py-24 bg-slate-900 overflow-hidden relative scroll-mt-20 text-white">
              <div className="max-w-7xl mx-auto px-6 relative z-10 text-left">
                <div className="mb-16 text-left text-white">
                  ...
                  <h2 className="text-base font-bold text-amber-400 uppercase tracking-widest mb-3 italic">Forfaits d'Intervention</h2>
                  <p className="text-3xl lg:text-5xl font-black mb-6 uppercase tracking-tighter italic text-white text-left">Solutions Prêtes à l'Emploi</p>
                  <p className="text-slate-400 max-w-2xl border-l-4 border-amber-400 pl-6 text-left italic leading-relaxed text-left">Pour des besoins ciblés et urgents, j'ai standardisé ces prestations sur la plateforme de confiance ComeUp.</p>
                </div>
            ...
            <section id="expertise" className="py-24 bg-white scroll-mt-20">
              <div className="max-w-7xl mx-auto px-6 text-slate-900 text-left">
                <div className="mb-16 text-left border-l-4 border-emerald-600 pl-8">
                  ...
                  <p className="text-slate-500 mt-4 italic max-w-2xl italic leading-relaxed text-left">Scénarios types de transformation de problématiques métier en solutions techniques performantes.</p>
                </div>
                <div className="grid md:grid-cols-3 gap-8 text-left text-left">
                  {expertiseCases.map((p, i) => (
                    <div key={i} className="bg-slate-50 rounded-[2.5rem] shadow-sm border border-slate-100 p-10 hover:shadow-xl transition-all group text-left">
              ...
              {!aiResult ? (
                aiUsageCount >= 3 ? (
                  ...
                  <div className="space-y-4 text-left">
                    <div className="flex items-center gap-2 text-emerald-600 font-bold text-xs uppercase tracking-widest mb-2 italic text-left"><Info size={14}/> Décrivez votre flux de travail actuel</div>
                    <textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-emerald-500 outline-none text-lg min-h-[160px] text-left italic" placeholder="Ex: Chaque lundi, je dois consolider 10 fichiers..."></textarea>
      ...
      <footer id="contact" className="py-24 bg-slate-900 text-white scroll-mt-20 text-left italic">
        <div className="max-w-7xl mx-auto px-6 text-left">
          <div className="grid md:grid-cols-2 gap-16 mb-20 text-left italic">
            <div className="text-left text-left">
              ...
              <div className="grid sm:grid-cols-2 gap-6 mt-12 text-white italic text-left">
                <a href="mailto:jc.data.solutions@outlook.fr" className="bg-white/5 p-6 rounded-2xl hover:bg-emerald-600 transition-all border border-white/5 group shadow-xl text-left text-left"><Mail className="mb-4 text-emerald-400 group-hover:text-white" /><div className="truncate text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white text-left">Email</div><div className="truncate text-sm font-black text-left text-left">jc.data.solutions@outlook.fr</div></a>
                <a href="https://www.linkedin.com/in/j%C3%A9r%C3%B4me-cassier-511601324/" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-blue-700 transition-all border border-white/5 group shadow-xl text-left text-left"><Linkedin className="mb-4 text-blue-400 group-hover:text-white" /><div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white text-left">LinkedIn</div><div className="text-sm font-black uppercase tracking-tighter italic text-white italic text-left text-left">Profil Pro</div></a>
                <a href="https://comeup.com/fr/@jerome-cassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-amber-500 transition-all border border-white/5 group shadow-lg text-left text-left"><ShoppingBag className="mb-4 text-amber-500 group-hover:text-white" /><div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white text-left">Prestations</div><div className="text-sm font-black italic uppercase text-white italic text-left text-left">Profil ComeUp</div></a>
                <a href="https://www.malt.fr/profile/jeromecassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-red-600 transition-all border border-white/5 group shadow-lg text-left text-left"><UserCheck className="mb-4 text-red-500 group-hover:text-white" /><div className="text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white text-left">Indépendant</div><div className="text-sm font-black uppercase tracking-tighter text-white italic text-left text-left">Profil Malt</div></a>
              </div>
            </div>
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden text-slate-900 text-left italic">
              {isSubmitted ? (
                ...
                <form className="space-y-4 text-left" onSubmit={handleContactSubmit}>
                  ...
                  <input required value={contactForm.name} onChange={(e) => setContactForm({...contactForm, name: e.target.value})} type="text" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-left" placeholder="Nom complet" />
                  <input required value={contactForm.email} onChange={(e) => setContactForm({...contactForm, email: e.target.value})} type="email" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-left" placeholder="Email professionnel" />
                  <textarea required value={contactForm.message} onChange={(e) => setContactForm({...contactForm, message: e.target.value})} rows="4" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-left" placeholder="Décrivez votre besoin métier..."></textarea>
          ...
          <div className="pt-12 border-t border-white/10 flex items-center justify-between text-slate-500 text-xs text-left">
            <div className="flex items-center gap-2 text-left">
              <Database className="text-emerald-500 w-5 h-5 text-left" />
              <span className="font-black text-white uppercase italic tracking-tighter italic text-left text-left">JC.DATA<span className="text-emerald-600">.SOLUTIONS</span></span>
              <button onClick={() => setCurrentPage('admin')} className="ml-4 opacity-10 hover:opacity-100 text-white transition-opacity italic text-left text-left"><Lock size={12} /></button>
            </div>
            <p className="italic uppercase tracking-widest text-[9px] italic font-bold text-left text-left">© {new Date().getFullYear()} - Expert Automation & Intelligence de Données.</p>
          </div>
        </div>
      </footer>
    </div>
  ...
  <div className="pt-32 pb-24 animate-in fade-in duration-700 bg-white min-h-screen text-slate-900 text-left italic text-left text-left text-left">
    <div className="max-w-4xl mx-auto px-6 text-left">
      ...
      <div className="mb-16 text-left text-left">
        ...
        <h1 className="text-4xl lg:text-6xl font-black text-slate-900 mb-6 leading-tight text-left uppercase italic text-left text-left text-left">{title} : <br/><span className={`text-${colorClass}-600`}>{subtitle}</span></h1>
        <p className="text-xl text-slate-600 leading-relaxed text-left italic leading-relaxed text-left text-left text-left">{description}</p>
      </div>
      <div className="bg-slate-900 rounded-[3rem] p-10 md:p-16 text-white mb-12 shadow-2xl border border-white/5 text-left italic text-left text-left">
          ...
          {examples.map((ex, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-8 items-start text-left text-left text-left">
              ...
              <div className="flex-1 text-left text-left text-left text-left">
                <h4 className={`text-xl font-bold mb-3 text-${colorClass}-400 uppercase tracking-tighter italic text-left text-left text-left`}>{i + 1}. {ex.title}</h4>
                <p className="text-slate-400 leading-relaxed mb-4 text-left italic text-left text-left text-left">"{ex.context}"</p>
                <div className="bg-white/5 p-6 rounded-2xl border border-white/10 text-left text-left text-left">
                  ...
                  <span className="text-slate-200 text-left block leading-relaxed italic text-left text-left text-left">{ex.solution}</span>
                </div>
              </div>
            </div>
          ))}

...
const comeupServices = [
  { id: 'r', title: "Rapprochement", description: "Identifier les écarts entre fichiers automatiquement.", price: "16,20 €", delay: "5j max", icon: Layers, url: "https://comeup.com/fr/service/513295/comparer-vos-fichiers-excel-et-identifier-les-ecarts-rapprochement" },
  { id: 'f', title: "Réparation", description: "Corriger les erreurs et restaurer vos fichiers corrompus.", price: "16,20 €", delay: "5j max", icon: ShieldCheck, url: "https://comeup.com/fr/service/512808/corriger-vos-erreurs-et-reparer-vos-fichiers-excel" },
  { id: 'c', title: "Conversion PDF", description: "Convertir vos PDF complexes en données Excel structurées.", price: "16,20 €", delay: "4j max", icon: FileSpreadsheet, url: "https://comeup.com/fr/service/512859/convertir-vos-fichiers-pdf-en-excel-et-automatiser-la-saisie-de-vos-donnees" },
  { id: 'd', title: "Dashboards", description: "Conception de tableaux de bord visuels et automatisés.", price: "16,20 €", delay: "6j max", icon: PieChart, url: "https://comeup.com/fr/service/512699/creer-votre-dashboard-excel-automatise-et-sur-mesure" },
  { id: 'n', title: "Nettoyage", description: "Suppression de doublons et normalisation de bases de données.", price: "16,20 €", delay: "4j max", icon: Database, url: "https://comeup.com/fr/service/512752/nettoyer-vos-bases-de-donnees-excel-suppression-doublons-normalisation" }
];
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          ...
          systemInstruction: { parts: [{ text: "Expert Excel VBA. Stratégie technique précise. Français." }] }
        })
      });
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 text-left text-slate-900">
                  {comeupServices.map((s) => (
                    <div key={s.id} className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 flex flex-col hover:bg-white/10 transition-all relative overflow-hidden group">
                      <div className="absolute top-0 right-0 bg-amber-500/20 text-amber-500 text-[10px] px-3 py-1 rounded-bl-2xl font-black">{s.delay}</div>
                      <div className="w-12 h-12 bg-amber-500/10 rounded-2xl flex items-center justify-center mb-6 text-amber-500 group-hover:scale-110 transition-transform"><s.icon size={24} /></div>
                      <h3 className="text-lg font-bold text-white mb-3 leading-tight flex-grow uppercase tracking-tight">{s.title}</h3>
                ...
                <div className="grid md:grid-cols-3 gap-8 text-left">
                  {projects.map((p, i) => (
                    <div key={i} className="bg-slate-50 rounded-[2.5rem] shadow-sm border border-slate-100 p-10 hover:shadow-xl transition-all group">
                      ...
                      <h3 className="text-2xl font-bold mb-4 group-hover:text-emerald-600 transition-colors uppercase italic tracking-tighter">{p.title}</h3>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden text-slate-900 text-left">
              {isSubmitted ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500 text-sm italic text-center">Sauvegardée dans ma base Cloud. Réponse sous 24h maximum.</p>
                  <button onClick={() => setIsSubmitted(false)} className="mt-8 text-emerald-600 font-bold hover:underline">Nouveau message</button>
                </div>
              ) : (
...
// --- TEMPLATE PAGES DÉTAILS ---
const DetailsTemplate = ({ title, subtitle, description, icon: Icon, examples, colorClass, goBack }) => (
  <div className="pt-32 pb-24 animate-in fade-in duration-700 bg-white min-h-screen text-slate-900 text-left">

...
// --- 1. DONNÉES STATIQUES (DÉFINIES EN PREMIER) ---
          ...
          {examples.map((ex, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-8 items-start text-left">
              ...
              <div className="flex-1 text-left text-white">
                <h4 className={`text-xl font-bold mb-3 text-${colorClass}-400 uppercase tracking-tighter italic`}>{i + 1}. {String(ex.title)}</h4>
                <p className="text-slate-400 leading-relaxed mb-4 italic">"{String(ex.context)}"</p>
...
const App = () => {
  ...
  // --- AUTH ET SYNCHRO FIRESTORE (BLINDÉ) ---
  useEffect(() => {
    const initApp = async () => {
      // 1. Authentification d'abord (Rule 3)
      try {
  ...
  // --- FETCH DATA (QUAND AUTH OK) ---
  useEffect(() => {
    if (!user || !db) return;

    // Écouteur Compteur Cadeau
    const settingsRef = doc(db, 'artifacts', appId, 'public', 'data', 'site_stats', 'gift_counter');
    const unsubGift = onSnapshot(settingsRef, (docSnap) => {
      if (docSnap.exists()) setCurrentGiftCount(docSnap.data().count || 0);
      else setDoc(settingsRef, { count: 3 }, { merge: true }).catch(() => {});
    }, (err) => console.log("Silent Error 9 handled"));

    // Écouteur Usage IA
    const usageDocRef = doc(db, 'artifacts', appId, 'users', user.uid, 'ai_usage', 'stats');
  ...
  const handleAiAction = async () => {
    ...
    try {
      ...
      const text = data.candidates?.[0]?.content?.parts?.[0]?.text || "Erreur de connexion IA.";
      setAiResult(String(text));
      if (db && user) {
        const aiRef = doc(db, 'artifacts', appId, 'users', user.uid, 'ai_usage', 'stats');
            ...
            <section id="services" className="py-24 bg-slate-50 scroll-mt-20 text-left text-slate-900">
              <div className="max-w-7xl mx-auto px-6 text-left">
                <div className="max-w-2xl mb-16 border-l-4 border-emerald-600 pl-8 text-left">
                  ...
                  <p className="text-3xl lg:text-4xl font-bold italic text-slate-900 text-left text-left">Solutions d'ingénierie Excel <span className="whitespace-nowrap italic text-slate-900 font-black">sur-mesure.</span></p>
                </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 text-slate-900">
                  {standardServices.map((s) => (
                    <div key={s.id} className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 flex flex-col hover:bg-white/10 transition-all group text-left text-white">
                      ...
                      <h3 className="text-lg font-bold text-white mb-3 leading-tight uppercase italic text-left">{String(s.title)}</h3>
                ...
                <div className="grid md:grid-cols-3 gap-8 text-left text-white">
                  {expertiseCases.map((p, i) => (
                    <div key={i} className="bg-slate-50 rounded-[2.5rem] shadow-sm border border-slate-100 p-10 hover:shadow-xl transition-all group text-left text-slate-900">
                      <div className="flex items-center gap-2 text-emerald-600 font-black text-xs uppercase mb-4 tracking-tighter italic text-left text-emerald-600"><CheckCircle2 size={16} /> {String(p.impact)}</div>
                      <h3 className="text-2xl font-bold mb-4 group-hover:text-emerald-600 transition-colors uppercase italic tracking-tighter text-left text-slate-900">{String(p.title)}</h3>
                 ...
                 <div className="grid md:grid-cols-3 gap-8 text-white text-left">
                    {methodology.map((m, i) => (
                      <div key={i} className="bg-white rounded-[2.5rem] p-10 shadow-2xl transform hover:-translate-y-2 transition-all text-slate-900 text-center flex flex-col items-center justify-center text-left text-slate-900">
                        ...
                        <h3 className="font-black text-slate-900 text-xl uppercase tracking-tighter mb-4 italic text-center">{String(m.title)}</h3>
                        <p className="text-slate-600 italic text-sm leading-relaxed text-center">"{String(m.content)}"</p>
                      </div>
                    ))}
                 </div>
               </div>
        ...
        <div className="max-w-7xl mx-auto px-6 text-left">
          <div className="grid md:grid-cols-2 gap-16 mb-20 text-left text-white text-left">
            <div className="text-left text-white text-left text-left">
              <h2 className="text-4xl font-bold mb-6 tracking-tight font-black uppercase leading-tight italic text-left text-white text-left font-black">Optimisez votre gestion. <br/><span className="text-emerald-500 underline decoration-emerald-500/20 underline-offset-8 italic text-left text-emerald-500">Commencez dès aujourd'hui.</span></h2>
              <div className="grid sm:grid-cols-2 gap-6 mt-12 text-white italic text-left text-left text-left">
                <a href="mailto:jc.data.solutions@outlook.fr" className="bg-white/5 p-6 rounded-2xl hover:bg-emerald-600 transition-all border border-white/5 group shadow-xl text-left text-white text-left text-white text-left">
                    ...
                    <div className="truncate text-xs font-black uppercase tracking-widest opacity-50 group-hover:opacity-100 text-white font-black text-left text-white font-black">Email</div>
                ...
                <a href="https://www.linkedin.com/in/j%C3%A9r%C3%B4me-cassier-511601324/" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-blue-700 transition-all border border-white/5 group shadow-xl text-left text-white text-left text-white text-left font-black">
                ...
                <a href="https://www.malt.fr/profile/jeromecassier" target="_blank" rel="noopener noreferrer" className="bg-white/5 p-6 rounded-2xl hover:bg-red-600 transition-all border border-white/5 group shadow-lg text-left text-white text-left text-white text-left text-left font-black">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden text-slate-900 text-left italic">
              {isSubmitted ? (
                <div className="text-center py-8 text-slate-900 text-center text-left text-center text-center">
                  ...
                  <h3 className="text-2xl font-bold mb-4 font-black uppercase tracking-tight italic text-slate-900 text-center text-slate-900 text-center text-left text-slate-900 text-center text-left font-black">Demande reçue !</h3>
                  <p className="text-slate-500 text-sm text-center italic text-center text-slate-500 text-center text-center text-center font-black">Réponse sous 24h maximum.</p>
                ...
                <form className="space-y-4 text-left text-slate-900 text-left text-left text-left text-left text-left text-left text-left text-left text-slate-900 font-black" onSubmit={handleContactSubmit}>
                  <h3 className="text-2xl font-bold mb-8 font-black tracking-tight text-left uppercase italic text-slate-900 italic underline decoration-emerald-500/20 text-left text-left text-left text-left text-left text-left font-black">Demander un audit gratuit</h3>
                  <input required value={contactForm.name} onChange={(e) => setContactForm({...contactForm, name: e.target.value})} type="text" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-slate-900 text-left font-black" placeholder="Nom complet" />
                  <input required value={contactForm.email} onChange={(e) => setContactForm({...contactForm, email: e.target.value})} type="email" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-slate-900 text-left font-black" placeholder="Email professionnel" />
                  <textarea required value={contactForm.message} onChange={(e) => setContactForm({...contactForm, message: e.target.value})} rows="4" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-emerald-500 font-bold italic text-left text-slate-900 text-left font-black" placeholder="Décrivez votre besoin métier..."></textarea>
                  <button type="submit" disabled={isSending} className="w-full bg-emerald-600 text-white font-black py-5 rounded-xl hover:bg-emerald-700 shadow-xl transition-all flex items-center justify-center gap-3 italic uppercase tracking-tighter text-white text-left text-white font-black">
          ...
          <div className="pt-12 border-t border-white/10 flex items-center justify-between text-white text-xs text-left text-white text-left text-left text-left text-left text-white font-black">
            <div className="flex items-center gap-2 text-left text-white text-left text-left text-white text-left">
              ...
              <button onClick={() => setCurrentPage('admin')} 
                className="ml-4 p-2 bg-white/5 border border-amber-400/30 rounded-lg text-amber-400 hover:bg-amber-400 hover:text-slate-900 transition-all shadow-lg">
                <Lock size={16} />
              </button>
            </div>
            <p className="italic uppercase tracking-widest text-[9px] italic font-bold text-left text-white font-black text-left text-white">© {new Date().getFullYear()} - Expert Automation & Intelligence de Données.</p>
          </div>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm text-slate-900 italic text-left text-left">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 text-left text-slate-900 text-left text-left text-left">
              <div className="text-left text-slate-900 text-left text-left text-left font-black">
                <h3 className="text-2xl font-bold flex items-center gap-2 font-black tracking-tight uppercase italic text-left text-slate-900 text-left text-left text-left text-left text-left text-left text-left"><Sparkles className="text-emerald-600 text-left text-emerald-600 text-left text-left font-black" /> Pré-Diagnostic IA ✨</h3>
              ...
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors text-left text-left text-left text-left font-black"><X size={24} /></button>
            </div>
            <div className="p-8 overflow-y-auto text-left italic text-slate-900 text-left text-left text-left text-left text-left text-left text-slate-900 font-black">
              {!aiResult ? (
                aiUsageCount >= 3 ? (
                  <div className="text-center py-6 animate-in zoom-in duration-500 text-slate-900 text-center text-left text-left text-slate-900 text-left text-left font-black">
                    <div className="w-20 h-20 bg-amber-100 text-amber-600 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-inner text-amber-600 text-center flex items-center justify-center text-center text-left text-left font-black"><Gift size={40} /></div>
                    ...
                    <p className="text-slate-600 mb-10 max-w-md mx-auto text-lg leading-relaxed text-center italic font-bold text-center text-slate-600 text-left text-left text-left text-left text-left font-black">Votre projet mérite une expertise humaine directe.</p>
                    <button onClick={() => { setIsModalOpen(false); navigateToSection('contact'); }} className="bg-emerald-600 text-white px-8 py-5 rounded-2xl font-black text-xl hover:scale-105 transition-all shadow-xl shadow-emerald-100 flex items-center justify-center gap-3 w-full uppercase tracking-tighter text-white text-left text-left font-black">Réserver mon audit gratuit <ArrowRight /></button>
                  </div>
                ) : (
                  <div className="space-y-4 text-left text-slate-900 text-left text-left text-slate-900 text-left text-left text-left text-left text-left text-slate-900 text-left font-black">
                    <div className="flex items-center gap-2 text-emerald-600 font-bold text-xs uppercase tracking-widest mb-2 italic text-emerald-600 text-left text-left text-left text-left text-left font-black"><Info size={14}/> Décrivez votre flux de travail actuel</div>
                    <textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-emerald-500 outline-none text-lg min-h-[160px] text-left italic text-slate-900 text-left text-left text-left text-left text-left font-black" placeholder="Ex: Chaque lundi, je fusionne 10 fichiers..."></textarea>
                    <button onClick={handleAiAction} disabled={isLoading || !userInput.trim()} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg hover:bg-emerald-600 transition-all flex items-center justify-center gap-3 shadow-lg uppercase tracking-tighter italic text-white text-white text-left font-black">
                      {isLoading ? <Loader2 className="animate-spin text-white" /> : <><Sparkles size={20} /> Analyser mon besoin</>}
                    </button>
                  </div>
                )
              ) : (
                <div className="space-y-6 text-left text-slate-900 text-left text-left text-slate-900 text-left text-left text-left text-left text-left text-slate-900 text-left font-black">
                  <div className="bg-emerald-50 p-8 rounded-[2rem] border border-emerald-100 whitespace-pre-wrap leading-relaxed text-left shadow-sm text-sm italic text-slate-800 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left text-left text-left font-black">
                    <div className="text-emerald-700 font-bold mb-4 flex items-center gap-2 border-b border-emerald-100 pb-2 uppercase text-xs tracking-widest not-italic italic underline decoration-emerald-200 italic font-black text-left text-emerald-700 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left text-left text-emerald-700 text-left text-left text-left text-left text-left text-left text-left font-black"><Sparkles size={14}/> Analyse Préliminaire</div>
                    {String(aiResult)}
                  </div>
                  <div className="p-6 bg-slate-900 rounded-[2rem] text-white flex flex-col sm:flex-row items-center gap-6 shadow-xl text-left text-white text-left text-left text-left text-left text-left text-left text-left text-left text-white text-left text-left text-left text-left text-left text-left text-left font-black">
                    <div className="flex-1 text-left text-white text-left text-left text-left text-left text-white text-left text-left text-left text-left text-white text-left text-left text-left text-left text-white text-left text-left text-left text-left font-black">
                        <div className="text-emerald-400 font-black uppercase text-xs tracking-widest mb-1 italic text-left text-emerald-400 font-black text-left text-left text-left text-left text-emerald-400 text-left text-left text-left text-left font-black">Valider cette analyse ?</div>
                        <p className="text-xs text-slate-400 italic leading-relaxed text-left text-slate-400 text-left text-left text-left text-left text-left text-left text-left text-slate-400 text-left text-left text-left text-left text-left text-left text-left text-slate-400 text-left text-left text-left text-left text-left text-left text-left text-slate-400 font-black">Échangeons 15 minutes pour confirmer le budget et la faisabilité.</p>
                    </div>
                    <button onClick={() => { setIsModalOpen(false); navigateToSection('contact'); }} className="bg-emerald-600 text-white px-6 py-4 rounded-xl font-black text-sm uppercase flex items-center gap-2 hover:bg-emerald-500 transition-all whitespace-nowrap italic text-white text-left text-left text-left text-left text-left text-left text-left text-left font-black"><MessageSquare size={16}/> Demander mon audit <ArrowRight size={16}/></button>
                  </div>
                  <button onClick={() => setAiResult("")} className="w-full py-3 text-slate-400 text-[10px] font-bold uppercase tracking-widest hover:text-slate-600 transition-colors italic underline italic text-center block w-full text-slate-400 text-left text-left text-left text-left text-left text-slate-400 text-left text-left text-left text-left text-left font-black">Autre analyse ({3 - aiUsageCount} restantes)</button>
                </div>
              )}
            </div>
          </div>
